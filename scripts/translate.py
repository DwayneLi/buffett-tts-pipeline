...
**首先判断原文段落类型**：
- 如果段落中有 `QUESTION C-Q:` 标记 → 按问答格式输出
- 如果段落中有 `[STATEMENT N]` 标记 → 这是巴菲特/芒格的开场陈述或独立发言，**直接以【巴菲特】或【芒格】开头翻译**，不要编造【问题】行，不要自行添加总结性提问
- 如果段落中有 `[CONTINUATION]` 标记 → 这是上一段发言的延续，**直接以对应角色标记（【巴菲特】或【芒格】）继续翻译**，不要编造【问题】行
- 如果段落既无 QUESTION 也无 STATEMENT 也无 CONTINUATION → 按角色直接翻译发言
...
- **【问题 C-Q】是格式占位符示例，永远不要在译文中输出这个字面字符串。** 如果原文没有具体的 QUESTION 编号，直接用角色标记（【巴菲特】/【芒格】）开头
"""


def build_user_prompt(text: str, glossary: list[str] = None, bg_topics: list[str] = None) -> str:
    """构建用户消息，可选附带术语对照表 + 已补充背景主题列表。"""
    prompt = "请翻译以下伯克希尔·哈撒韦股东大会问答：\n\n"
    if glossary:
        prompt += "【已建立的术语对照表】（后续翻译请严格遵循这些简称，不要重复给出英文原名或重复解释）\n"
        prompt += "\n".join(glossary) + "\n\n"
    if bg_topics:
        prompt += "【已补充背景的主题】（这些主题此前已有背景，绝对不要再补充背景，直接翻译问答即可）\n"
...
def number_questions(text: str) -> str:
    ...
    for line in lines:
        ...
        if chap_match:
            # 上一章节没有提问者 → 回补 STATEMENT 标记
            if not chapter_has_questions and chapter_num >= 1:
                # 在 chapter 标题后插入 STATEMENT 标记
...
def split_transcript(text: str, max_chunk_chars: int = 4000) -> list[str]:
    ...
    """
    # 1) 按预注入的 QUESTION 标记切分（保证不切断 Q&A 对）
    qa_pattern = r'(?=QUESTION\s+\d+-\d+:)'
...
def translate_file(input_path: str, output_path: str) -> None:
    ...
    for i, chunk in enumerate(chunks):
        ...
        if bg_topics:
            extra_info.append(f"已补背景: {len(bg_topics)} 个主题")
        extra_str = f" [{', '.join(extra_info)}]" if extra_info else ""

        print(f"\n{'='*50}")
        print(f"🔄 翻译第 {i+1}/{len(chunks)} 段 ({len(chunk)} 字符){extra_str}...")

        try:
            result = call_llm(
                SYSTEM_PROMPT,
                build_user_prompt(chunk, glossary if glossary else None, bg_topics if bg_topics else None)
            )
            all_results.append({
                "index": i,
